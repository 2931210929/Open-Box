...
- **直连和代理切换重启内核**：后端设置里的开关（默认开）。开着时，站点集在直连和代理之间切换会在后台重启内核，已建立的连接会断一下；关掉后改用「开关规则集」热切换——切换不重启内核，已建立的连接不断，新连接和 DNS 在 1 秒内按新线路走。设备没有 nftables 重定向（纯 tun）且入口旁路随之变化时，仍按老办法重启。改这个开关本身要重启内核一次。
