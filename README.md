...
- **直连和代理切换重启内核**：后端设置里的开关（默认关）。关着时用「开关规则集」热切换——站点集在直连和代理之间切换不重启内核，已建立的连接不断，新连接和 DNS 在 1 秒内按新线路走；打开后回到老做法，每次切换都在后台重启内核，已建立的连接会断一下。设备没有 nftables 重定向（纯 tun）且入口旁路随之变化时，仍按老办法重启。改这个开关本身要重启内核一次。
